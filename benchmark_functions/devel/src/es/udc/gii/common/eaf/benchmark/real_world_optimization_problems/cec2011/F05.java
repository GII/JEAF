 ...
 */
public class F05 extends BenchmarkObjectiveFunction {

    private double R1 = 3.0;
    
    private double R2 = 0.2;
    
    private double A = 3.2647e+3;
    
    private double B = 9.5373e+1;

    private double lemda1 = 3.2394;

    private double lemda2 = 1.3258;

    private double lemda3 = 1.3258;

    private double c = 4.8381;

    private double d = 2.0417;

    private double n1 = 22.956;
    
    private double gama = 0.33675;
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (int j = 0; j < N; j++) {
                    ...
                    for (int k = 0; k < N; k++) {
                        ...
                        ctheta_ijk = (rd1*rd1+rd2*rd2-rd3*rd3*rd3)/(2*rd1*rd2);
                        G_th_ijk =1+(c*c)/(d*d)-(c*c)/(d*d+(h-ctheta_ijk)*(h-ctheta_ijk));
                        jeta[i][j] += fcr[i][k]*G_th_ijk*Math.exp(
                                Math.pow(this.lemda3, 3.0)*Math.pow(r[i][j] - r[i][k], 3.0));
