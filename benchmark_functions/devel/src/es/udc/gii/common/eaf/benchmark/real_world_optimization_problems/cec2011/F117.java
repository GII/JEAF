 ...
 */
public class F117 extends ELDFunction {

    public F117() {
        super();
        this.setNo_of_Units(140);

        double[] l_b = {71, 120, 125, 125, 90, 90, 280, 280, 260, 260, 260, 260,
            260, 260, 260, 260, 260, 260, 260, 260, 260, 260, 260, 260, 280, 280,
            280, 280, 260, 260, 260, 260, 260, 260, 260, 260, 120, 120, 423, 423, 3,
            3, 160, 160, 160, 160, 160, 160, 160, 160, 165, 165,
            165, 165, 180, 180, 103, 198, 100, 153, 163, 95, 160, 160, 196, 196, 196, 196,
            130, 130, 137, 137, 195, 175, 175, 175, 175, 330,
            160, 160, 200, 56, 115, 115, 115, 207, 207, 175, 175, 175, 175, 360, 415, 795,
            795, 578, 615, 612, 612, 758, 755, 750, 750, 713,
            718, 791, 786, 795, 795, 795, 795, 94, 94, 94, 244, 244, 244, 95, 95, 116,
            175, 2, 4, 15, 9, 12, 10, 112, 4, 5,
            5, 50, 5, 42, 42, 41, 17, 7, 7, 26
        };
        this.setLower_bounds(l_b);

        double[] u_b = {119, 189, 190, 190, 190, 190, 490,
            490, 496, 496, 496, 496, 506,
            509, 506, 505, 506, 506, 505, 505, 505,
            505, 505, 505, 537, 537,
            549, 549, 501, 501, 506, 506, 506, 506,
            500, 500, 241, 241, 774,
            769, 19, 28, 250, 250, 250, 250, 250,
            250, 250, 250, 504, 504,
            504, 504, 471, 561, 341, 617, 312, 471,
            500, 302, 511, 511, 490,
            490, 490, 490, 432, 432, 455, 455, 541,
            536, 540, 538, 540, 574,
            531, 531, 542, 132, 245, 245, 245, 307,
            307, 345, 345, 345, 345,
            580, 645, 984, 978, 682, 720, 718, 720,
            964, 958, 1007, 1006, 1013,
            1020, 954, 952, 1006, 1013, 1021, 1015, 203,
            203, 203, 379, 379, 379,
            190, 189, 194, 321, 19, 59, 83, 53,
            37, 34, 373, 20, 38,
            19, 98, 10, 74, 74, 105, 51, 19,
            19, 40};
        this.setUpper_bounds(u_b);

        this.setPower_Demand(49342.0);
        ...
        double[][] d_1_aux = new double[140][7];
        int d_3_index = 0;
        for (int i = 0; i < d_1_aux.length; i++) {

            d_1_aux[i][0] = d_1[i][0]; //Pmin
            d_1_aux[i][1] = d_1[i][1]; //Pmax
            d_1_aux[i][2] = d_1[i][4]; //a
            d_1_aux[i][3] = d_1[i][3]; //b
            d_1_aux[i][4] = d_1[i][2]; //c

            if (d_3_index < d_3.length && i == (int) d_3[d_3_index][0] - 1) {

                d_1_aux[i][5] = d_3[d_3_index][1]; //e;
                d_1_aux[i][6] = d_3[d_3_index][2]; //f
                d_3_index++;
            }
        }

        this.setData_1(d_1_aux);
        ...
        double[][] d_2_aux = new double[140][9];
        int d_4_index = 0;
        for (int i = 0; i < d_2_aux.length; i++ ) {
            d_2_aux[i][0] = d_2[i][0];
            d_2_aux[i][1] = d_2[i][1];
            d_2_aux[i][2] = d_2[i][2];
            if (d_4_index < d_4.length && i == (int) d_4[d_4_index][0]) {
                d_2_aux[i][3] = d_4[d_4_index][1];
                d_2_aux[i][4] = d_4[d_4_index][2];
                d_2_aux[i][5] = d_4[d_4_index][3];
                d_2_aux[i][6] = d_4[d_4_index][4];
                d_2_aux[i][7] = d_4[d_4_index][5];
                d_2_aux[i][8] = d_4[d_4_index][6];
                
            }
        }
        
        this.setData_2(d_2_aux);

        double[][] b_1 = new double[140][140];


        this.setB1(b_1);
        double[] b_2 = new double[140];
        this.setB2(b_2);
        this.setB3(0.0);

        this.setPower_balance_penalty_factor(1.0e+7);
        this.setCapacity_limit_penalty_factor(1.0e+5);
        this.setRamp_limit_penalty_factor(1.0e+7);
