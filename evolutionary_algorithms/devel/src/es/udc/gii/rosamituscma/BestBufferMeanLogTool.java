...
import es.udc.gii.common.eaf.log.*;
 ...
 * @since 1.0
 */
public class BestBufferMeanLogTool extends LogTool {

    /** Creates a new instance of BestMeanLogTool */
    public BestBufferMeanLogTool() {
    }
    ...
    public void update(Observable o, Object arg) {
        ...
        if (algorithm.getState() == EvolutionaryAlgorithm.REPLACE_STATE && arg == null) {
            best = bestSpec.get(((ConstrainedMotionEvolutionaryAlgorithm)algorithm).getBufferPopulation(), 1, algorithm.getComparator()).get(0);

            super.getLog().println(
                    algorithm.getGenerations() + " - " +
                    best.getFitness() + " - " +
                    FitnessUtil.meanFitnessValue(
                    ((ConstrainedMotionEvolutionaryAlgorithm)algorithm).getBufferPopulation()));
        }
    }

    @Override
    public String getLogID() {
        return "bestbuffermean";
    }
}
