 ...
 * @since 1.0
 */
public class FesPlugin extends StopTestPlugin {

    /**
     * Maximum number of function evaluations
     */
    private int max_fes = -Integer.MAX_VALUE;
    ...
    public int getMax(EvolutionaryAlgorithm algorithm) {
        ...
        if (this.max_fes == -Integer.MAX_VALUE) {
            
            list_test = this.getStopTestList(algorithm);
            
            for (StopTest s : list_test) {
                
                if (s instanceof FEsStopTest) {
                    this.max_fes = ((FEsStopTest)s).getFEs(algorithm);
                }
            ...
            if (this.max_fes == -Integer.MAX_VALUE) {
                throw new ConfigurationException("Wrong Plugin configuration: " +
                        "If you use FEsPlugin you must configure a FEsStopTest");
            }
